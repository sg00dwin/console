...
// import { VerticalPodAutoscalerModel } from '../models';
...
const VerticalPodAutoscalersReference: K8sResourceKindReference = 'VerticalPodAutoscaler';

// const { common } = Kebab.factory;
// const menuActions = [...Kebab.getExtensionsActionsForKind(VerticalPodAutoscalerModel), ...common];
...
export const VerticalPodAutoscalersDetails: React.FC<VerticalPodAutoscalersDetailsProps> = ({
  // obj: hpa,
  obj: vpa,
}) => {
  const { t } = useTranslation();
  return (
    <>
      <div className="co-m-pane__body">
        <SectionHeading text={t('public~VerticalPodAutoscaler details')} />
        <div className="row">
...
const pages = [
  navFactory.details(DetailsForKind(VerticalPodAutoscalersReference, (obj) => <h1>Hello</h1>)),
  // navFactory.editYaml(),
  // navFactory.events(ResourceEventStream),
];
export const VerticalPodAutoscalersDetailsPage: React.FC<VerticalPodAutoscalersDetailsPageProps> = (
  props,
) => (
  <DetailsPage
    {...props}
    kind={VerticalPodAutoscalersReference}
    // menuActions={menuActions}
    pages={pages}
  />
);
VerticalPodAutoscalersDetailsPage.displayName = 'VerticalPodAutoscalersDetailsPage';
...
// const kind = 'VerticalPodAutoscaler';

const VerticalPodAutoscalersTableRow: React.FC<RowFunctionArgs<K8sResourceKind>> = ({ obj }) => {
  return (
    <>
...
const VerticalPodAutoscalersList: React.FC = (props) => {
  const { t } = useTranslation();
  const VerticalPodAutoscalersTableHeader = () => [
    {
      title: t('public~Name'),
  ...
  return (
    <Table
      {...props}
      aria-label={t('public~VerticalPodAutoScalers')}
      Header={VerticalPodAutoscalersTableHeader}
      Row={VerticalPodAutoscalersTableRow}
      virtualize
    />
  );
};
VerticalPodAutoscalersList.displayName = 'VerticalPodAutoscalersList';

export const VerticalPodAutoscalersPage: React.FC<VerticalPodAutoscalersPageProps> = (
  props,
) => (
  <ListPage
    {...props}
    kind={VerticalPodAutoscalersReference}
    ListComponent={VerticalPodAutoscalersList}
    canCreate={false}
  />
);
VerticalPodAutoscalersPage.displayName = 'VerticalPodAutoscalersListPage';

export type VerticalPodAutoscalersDetailsProps = {
  obj: VerticalPodAutoscalerKind;
};

export type VerticalPodAutoscalersPageProps = {
  showTitle?: boolean;
  namespace?: string;
  selector?: any;
};

export type VerticalPodAutoscalersDetailsPageProps = {
  match: any;
};
