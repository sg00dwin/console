---
name: start-story
description: Bootstrap or resume a structured, phased workflow for a CONSOLE story — from Jira intake and reconnaissance through planning, implementation, verification, and self-audit. Manages branch creation, documentation tracking, and phase-by-phase progress.
...
trigger: start-story, story, console story
---

# /start-story Skill – Structured Story Workflow

**Purpose:**  
This skill sets up (or resumes) a clean, trackable environment for implementing CONSOLE story issues and helps the user seamlessly continue at the appropriate phase.

When invoked, follow these steps in order:

### Step 0: Validate Input

If no CONSOLE issue key was provided as an argument:
  - Ask the user: "Which CONSOLE story should I work on? Please provide the issue key (e.g., CONSOLE-1234)."
  - Wait for their response.

### Step 1: Gather ALL Issue Context

The goal of this step is to retrieve **every piece of information** provided in the story — the full description, all comments, all attachments, and all linked supporting files. Missing context risks an incomplete implementation.
...
3. If the description appears truncated (ends mid-sentence, ends with "See", or is missing expected sections), note this for the validation gate in Step 1d.

#### 1b: Fetch Comments, Attachments, and Linked Issues

**Always run this step** — comments and attachments frequently contain critical context (design docs, mockups, acceptance criteria details, Drive links) not present in the description:
...
4. Attempt to download each file using `mcp__google-docs__drive_files_download` with a `savePath` under `.claude/local/devnotes/CONSOLE/<number>-<Short-Title>/supporting-files/` (create the directory first with `Bash(mkdir -p ...)`).
5. **Read each downloaded text file** and incorporate the content into the issue summary.
...
**Jira issue links:** The description may contain inline Jira issue cards that get silently dropped during ADF-to-text conversion — appearing as orphaned text like "See" with no link. Use `mcp__jira__jira_get_issue_links` to fetch all linked issues. For each linked issue, fetch its comments and attachments to check for additional supporting files or Drive links.
...
Before proceeding, verify that all information was captured:

1. **Check for reference gaps:** If the description or comments mention "See", "attached", "screenshot", "design doc", "mockup", "Google Drive", "folder", or similar keywords, confirm that corresponding links were found AND their content was retrieved.
2. **If gaps are detected** (references to files but no links found, or links found but retrieval failed):
   - **Stop and ask the user:** "The story references supporting files but I couldn't retrieve them. Can you provide the links or paste the content directly?"
...
1. Determine target branch name: `CONSOLE-<number>-<Short-Title>`
...
3. **Handle uncommitted changes (Safety First):**
   - If there are **any uncommitted changes** (modified, staged, or untracked files):
     - Immediately pause the skill and inform the user with this exact message:
       ...
       > 2. Stash them, then  
       > 
       > Once you're ready, re-run `/start-story` and I will continue from where we left off."
...
4. **If the working tree is clean** (no uncommitted changes):
   - Check if a branch matching `CONSOLE-<number>-*` already exists.
   - **If the branch already exists:**
     - Inform the user: "Branch `CONSOLE-<number>-<Short-Title>` already exists."
     - Checkout the existing branch: `git checkout CONSOLE-<number>-<Short-Title>`
   - **If the branch does not exist:**
     - Ask the user: "What short title should I append to the branch name? (It will be named `CONSOLE-<number>-<Short-Title>`)"
     - Wait for their response.
     - Create the branch from the latest `upstream/main`.
**Rule:** Never delete, reset, or overwrite existing branches.

### Step 3: Devnotes Dossier Management

The dossier is a living document updated as work proceeds through planning, implementation, and verification phases.

1. Dossier path = `.claude/local/devnotes/CONSOLE/<number>-<Short-Title>/DOSSIER.md`
2. **If DOSSIER.md already exists:**
   - Inform the user.
   - Read the full dossier.
   - Analyze and summarize current progress.
3. **If DOSSIER.md does not exist:**
   - Create the directory `.claude/local/devnotes/CONSOLE/<number>-<Short-Title>/`
   - Copy the content from the template:  
    `.claude/skills/start-story/DOSSIER-TEMPLATE.md`
   - Populate frontmatter with Jira details (story number, title, description, acceptance criteria, related, etc.).
...
00-Recon.md         # Phase 0: Detailed reconnaissance findings
01-Plan.md          # Phase 1: Detailed implementation plan
02-Execution.md     # Phase 2: Execution notes and decisions
03-Verification.md  # Phase 3: Verification evidence and test results
04-Audit.md         # Phase 4: Self-audit findings
05-Report.md        # Phase 5: Final report
```
...
3. Offer context-aware next steps:
   - If early in the process: Ask if they want to start **Phase 0: Reconnaissance & Mental Modeling**.
   - If already in progress: Ask whether to continue the current phase, re-run a phase, or jump to another.
   - If advanced: Offer to proceed with Self-Audit (Phase 4) or Final Report (Phase 5).
